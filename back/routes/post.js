...
router.post('/images', isLoggedIn, upload.array('image'), (req, res) => {
  ...
  res.json(req.files && req.files.map(v => v.filename));
});
...
router.post('/:id/retweet', async(req, res, next) => {
  try{
    ...
    if (!post){
      return res.status(404).send('Not post');
    }
    if(req.user.id === post.UserId || (post.Retweet && post.Retweet.UserId === req.user.id)){
      return res.status(403).send('Not retweet own');
    }
    ...
    if(exPost){
      return res.status(403).send('already done');
    }
